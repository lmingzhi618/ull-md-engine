...
void print_usage(const char* prog) {
    std::cerr << "usage: " << prog
              << " <consumers> <messages> <capacity> <affinity> <warmup> "
                 "[--json|--text]\n"
              ...
              << " --json   : emit canonical benchmark JSON v1(default)\n"
...
bool is_valid_output_mode(const std::string& mode) {
...
int main(int argc, char** argv) {
    ...
    const std::string output_mode = argc > 6 ? argv[6] : "--json";

    if (argc > 7 || !is_valid_output_mode(output_mode)) {
        print_usage(argv[0]);
        return 1;
    }
    const bool text_output = output_mode == "--text";
    ...
    for (std::uint64_t consumer_id = 0; consumer_id < consumers;
        ...
        consumer_threads.emplace_back([&, consumer_id] {
            ...
            for (std::uint64_t expected = 0; expected < messages; ++expected) {
                ...
                // Fanout backpressure:
                // producer capacity is controlled by the slowest consumer
                // sequence.
    ...
    if (!text_output) {
        ...
        std::cout << "hist_unit=ns bucket_ns=" << kBucketNs
                  << " max_ns=" << kMaxNs << "\n";

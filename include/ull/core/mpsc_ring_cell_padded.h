...
template <class T> class MpscRingCellPadded {
  static_assert(std::is_trivially_copyable_v<T>,
                "MpscRingPadded<T> requires T to be trivially copyable");

public:
  explicit MpscRingCellPadded(std::size_t capacity_pow2)
...
private:
  ...
  static_assert(alignof(Cell) == kCacheLine);
  ...
  alignas(kCacheLine) std::uint64_t tail_{0};
};
} // namespace ull

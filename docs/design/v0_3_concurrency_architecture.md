...
Correctness tests should be run with Debug builds so that `assert` checks are enabled. Release builds are reserved for benchmarks because `NDEBUG` disables `assert`.
